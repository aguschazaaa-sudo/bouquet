...
subir, publicar— cuando para el que lo usa es un solo acto. Los tres están en
*Lo que quedó abierto*, cada uno con su medición y su disparador. **Se
asentaron sin repararlos, a propósito:** el del CORS pide un otorgamiento de
permiso, y los otros dos son decisiones de diseño — no parches adentro de la
tarea que los encontró.
...
| Qué | Por qué | Quién |
|---|---|---|
| ⚠️ **La callable `procesarFoto` NO es alcanzable desde el navegador: el preflight da 403, y el panel lo muestra como error de CORS** | Lo vio el dueño usando el panel; medido después con `curl` crudo el 2026-09-22, y el `ACTIVE` de la API de Cloud Functions **no lo veía**. `OPTIONS` a `https://us-central1-bouquet-vinos.cloudfunctions.net/procesarFoto` con `Origin` y `Access-Control-Request-Method: POST` devuelve **403 Forbidden** de `Google Frontend`, **sin un solo header `Access-Control-Allow-*`** — y sin ese header no hay navegador que siga. **Control negativo:** el mismo `OPTIONS` contra una function inventada da **404**, así que el 403 es de esta function, no del frontend contra cualquier `OPTIONS`. **El control que aísla la causa:** un `POST` anónimo devuelve **el mismo 403 HTML**, no el JSON `UNAUTHENTICATED` que devolvería la callable — **el código nunca corre**, lo frena IAM antes, y por eso el `cors: true` que `onCall` trae por defecto no llega a ejecutarse. Falta `allUsers` como `roles/run.invoker` en el servicio de Cloud Run. **Lo que NO es, y también está medido: no es el bucket.** El endpoint de subida contesta el preflight con `Access-Control-Allow-Origin: *`, y una descarga con `Origin` también, **aunque el bucket no tiene política CORS** —la API JSON de Storage lo devuelve sin campo `cors`—: `gsutil cors set` no arregla nada acá y agregarlo sería ruido. **Consecuencia hoy: el crudo se sube y queda huérfano en CADA intento**, no en un fallo raro; corrige al pendiente que decía *"cuando pesen"*. **Repararlo es un otorgamiento de permiso** — el mismo "Permission Grant" que frena a 4.3 —, así que lo corre una persona. **Disparador: bloquea HU-04.1 entera, la foto no se puede subir.** Desde 2026-09-22. | el usuario + `functions` |
| ⚠️ **El formulario del vino usa mal el espacio: una columna de 640 px y el resto de la pantalla vacío** | Lo vio el dueño mirándolo, que es la única forma de juzgarlo — CLAUDE.md: compilar, pasar tests y desplegarse son tres cosas distintas de que **alguien lo haya mirado renderizado**. En el código es `formulario_del_vino.dart:128-135`: `ListView` → `Center` → `ConstrainedBox(maxWidth: 640)` → una `Column` con cuatro bloques apilados (el vino, la venta, las fotos, el pie). El panel corre en **web y en Android con el mismo ancho**, así que en un monitor el alta se recorre scrolleando una columna angosta mientras los costados no hacen nada. No se tocó en esta sesión a propósito: **qué va al lado de qué es dirección visual** y el proyecto la decide con `/disenio` antes de escribir widgets, no subiéndole el `maxWidth` a un `ConstrainedBox`. **Disparador:** la próxima tarea que toque `presentation/` del panel, y antes de que el dueño cargue el catálogo real. Desde 2026-09-22. | el dueño + `admin-presentacion` |
| ⚠️ **Cargar un vino con su foto son TRES gestos, y tiene que ser uno** | El dueño lo dijo con las palabras del que lo usa: *"no tiene sentido primero cargar el vino, para después subir la foto, para después activar"*. Está medido en el código: `SeccionDeFotos` recibe `productoId: widget.original?.id` (`formulario_del_vino.dart:155-157`), que en un alta es `null`, y `seccion_de_fotos.dart:76` muestra *"Guardá el vino primero"*; y el pie avisa *"se carga sin publicar y sin stock"* (`pie_del_formulario.dart:74`), así que publicar es un tercer viaje. **La causa está en de dónde salió el diseño:** la ruta de Storage y el `arrayUnion` necesitan el id del producto ([ADR 015](architecture/decisions/015-fotos-del-panel.md)), o sea que el flujo se razonó desde lo que el SDK necesita y no desde lo que hace una persona con una botella nueva en la mano. **Hay salidas y acá NO se elige ninguna:** reservar el id antes de guardar, subir el crudo a una ruta de borrador y moverlo, o que el alta haga las tres cosas al confirmar. Es un change con su ADR, no un parche adentro de otra tarea. **Disparador:** antes de que el dueño cargue el catálogo real — es el hito 1, y el defecto se multiplica por cada vino. Desde 2026-09-22. | el dueño + `admin-presentacion` + `functions` |
...
| **Los crudos huérfanos en Storage** si `procesarFoto` falla a mitad de camino: no son alcanzables y no rompen nada. ⚠️ **Corregido el 2026-09-22: mientras el preflight de la callable dé 403, se produce uno en CADA intento de subida**, no en un fallo raro — la subida sí entra y la callable nunca corre | Cuando pesen, y hay que barrer los que deje el 403 | 2026-09-22 |
